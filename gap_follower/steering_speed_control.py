...
class SteeringSpeedNode(Node):
    ...
    def filter_scan_cb(self, msg:LaserScan):
        ...
        self.theta = self.get_theta_target(poss_edges=self.possible_edges)
    ...
    def follow_the_gap(self):
        ...
        for tub in self.possible_edges:
            string += str(tub)
        self.get_logger().info(f"theta: {self.theta} deg -- {string}")
